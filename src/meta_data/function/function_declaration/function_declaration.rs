...
use crate::{meta_data::{function::{argument_info::argument_info::ArgumentInfo, function_modifiers::FunctionModifiers}, soul_type::generic::Generic, type_meta_data::{self, TypeMetaData}}, tokenizer::token::TokenIterator};
    ...
    pub fn are_arguments_compatible(
        ...
        for other_arg in other_optionals {
            match self.optionals.get(&other_arg.name) {
                Some(val) => {
                    if let Err(_) = val.are_compatible(iter, other_arg, type_meta_data, generics) {
                        return false;
                    }
                },
                None => return false,
