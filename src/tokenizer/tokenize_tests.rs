...
fn test_tokenize_backslash() {
	...
	assert_eq!(result_tokens.unwrap_err().to_err_message(), "at 1:4; !!error!! '\\' can only be placed at the end of a line");
	...
	let result_tokens = tokenize_file(source_file, 10, &mut meta_data);
	assert!(result_tokens.is_err());
	assert_eq!(result_tokens.unwrap_err().to_err_message(), "at 1:9; !!error!! '\\' can only be placed at the end of a line");
}
