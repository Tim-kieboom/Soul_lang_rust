...
use crate::{run_steps::{parser, source_reader, tokenizer, RunStepsInfo}, utils::{logger::LogOptions, node_ref::MultiRef, time_logs::TimeLogs}, MainErrMap};
...
pub fn cache_files(run_option: &Arc<RunOptions>, logger: &Arc<Logger>, time_log: &MultiRef<TimeLogs>) {
    ...
    if let Err(err) = cache_file(run_option.clone(), sub_files, Path::new(&run_option.file_path), logger.clone(), time_log.clone()) {
...
fn cache_all_subfiles(run_option: Arc<RunOptions>, subfiles_tree: Arc<SubFileTree>, logger: &Arc<Logger>, time_log: &MultiRef<TimeLogs>) {
    ...
    for file in subfiles.iter() {
        ...
        pool.execute(move || {
            let result = cache_file(run_option, tree, Path::new(&file), log, t_log);
...
fn cache_file<'a>(run_option: Arc<RunOptions>, sub_files: Option<Arc<SubFileTree>>, file_path: &Path, logger: Arc<Logger>, time_log: MultiRef<TimeLogs>) -> Result<()> {
