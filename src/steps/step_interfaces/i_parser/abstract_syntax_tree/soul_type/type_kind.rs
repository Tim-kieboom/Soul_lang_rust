...
impl TypeKind {
    ...
    pub fn to_name_string(&self) -> String {
        match self {
            ...
            TypeKind::Function(function_signature) => function_signature.borrow().node.name.0.clone(),
            ...
            TypeKind::TraitType(kind) => match kind {
                TraitKind::Indexed(indexed) => format!("TraitType<{}[{}]>", indexed.collection.to_string(), indexed.index.to_string()),
                TraitKind::Meth(Meth{methode}) => format!("TraitType<{}.{}()>", methode.callee.node.to_string(), methode.name.0)
            },
        }
    }
