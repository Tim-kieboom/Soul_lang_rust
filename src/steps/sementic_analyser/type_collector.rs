    ...
    fn visit_global_node(&mut self, node: &mut GlobalNode) {
        ...
        match &mut node.node {
            GlobalKind::ClassDecl(multi_ref) => {
                let mut class_decl = multi_ref.borrow_mut();
            ...
            GlobalKind::StructDecl(struct_ref) => {
                let struct_decl = struct_ref.borrow();
    ...
    fn check_var_decl(&mut self, variable_kind: &mut VariableKind, span: SoulSpan) {
        match variable_kind {
            VariableKind::Variable(multi_ref) => {
                if multi_ref.borrow().ty.is_none_type() {
                    
                    let ty = if let Some(init) = &multi_ref.borrow().initializer {
    ...
    fn check_assignment(&mut self, assignment: &mut Assignment, span: SoulSpan) {
        ...
        match &assignment.target.node {
            ...
            ExprKind::Variable(variable) => {
                ...
                if !var_ref.borrow().ty.is_none_type() {
                    return;
                }

                if var_ref.borrow().ty.is_none_type() { 
                    var_ref.borrow_mut().ty = match try_get_exprkind_ty(&assignment.value.node, self.get_scope()) {
                        Ok(val) => match val {
                            Some(ty) => ty,
                            None => {
...
fn try_get_exprkind_ty(expr: &ExprKind, scope: &ScopeVisitor) -> Result<Option<SoulType>, String> {
    ...
    fn from_variable(variable: &Ident, scope: &ScopeVisitor) -> Result<Option<SoulType>, String> {
        match scope.lookup(&variable.0)
            ...
            .ok_or(format!("var: '{}' not found", variable.0))? 
        {
            ScopeKind::Variable(multi_ref) => Ok(Some(multi_ref.borrow().ty.clone())),
            _ => unreachable!(),
        }
    }
    ...
    match expr {
        ...
        ExprKind::UnwrapVarDecl(variable_kind) => {
            match variable_kind.as_ref() {
                VariableKind::Variable(multi_ref) => Ok(Some(multi_ref.borrow().ty.clone())),
        ...
        ExprKind::Lambda(lambda_decl) => Ok(lambda_decl.signature.borrow().return_type.clone()),
        ...
        ExprKind::NamedTuple(named_tuple) => {
            ...
            for (name, expr) in &named_tuple.values {
                tuple_kind.insert(
                    name.clone(),
                    try_get_exprkind_ty(&expr.node, scope)?
                        .ok_or(format!("could not get type from expression: '{}'", expr.node.to_string(0)))?
                );
            }
        ...
        ExprKind::ArrayFiller(array_filler) => {
            let mut ty = try_get_exprkind_ty(&array_filler.fill_expr.node, scope)?
                .ok_or(format!("could not get type from expression: '{}'", array_filler.fill_expr.node.to_string(0)))?;
            ty.wrappers.push(TypeWrapper::Array);
            Ok(Some(ty))
        },
    }
...
fn field_type_from_type(base: &TypeKind, field_var: &Variable, scope: &ScopeVisitor) -> Result<Option<SoulType>, String> {
    ...
    match &base {
        ...
        TypeKind::Function(multi_ref) => match &multi_ref.borrow().node.return_type {
            Some(ty) => field_type_from_type(&ty.base, field_var, scope),
            None => Err(format!("tryed to index from function: '{}', but function has not return type", multi_ref.borrow().node.name.0)),
        },
        ...
        TypeKind::ExternalType(spanned) => {
            ...
            for kind in header.scope.get(&spanned.node.path.0).ok_or(format!("path: '{}' not found", spanned.node.path.0))?.iter() {
                match kind {
                    ScopeKind::Enum(multi_ref) => return field_type_from_type(&TypeKind::Enum(multi_ref.borrow().name.clone()), field_var, scope),
                    ScopeKind::Class(multi_ref) => return field_type_from_type(&TypeKind::Class(multi_ref.borrow().name.clone()), field_var, scope),
                    ScopeKind::Union(multi_ref) => return field_type_from_type(&TypeKind::Union(multi_ref.borrow().name.clone()), field_var, scope),
                    ScopeKind::Trait(multi_ref) => return field_type_from_type(&TypeKind::Trait(multi_ref.borrow().name.clone()), field_var, scope),
                    ScopeKind::Struct(multi_ref) => return field_type_from_type(&TypeKind::Struct(multi_ref.borrow().name.clone()), field_var, scope),
                    ScopeKind::TypeEnum(multi_ref) => return field_type_from_type(&TypeKind::TypeEnum(multi_ref.borrow().name.clone(), multi_ref.borrow().types.clone()), field_var, scope),
                    ScopeKind::TypeDefed(multi_ref) => return field_type_from_type(&TypeKind::TypeDefed(multi_ref.borrow().name.clone()), field_var, scope),
                    _ => (),
                }
            }
        ...
        _ => Err(format!("typeKind: '{}' can not have fields", base.get_variant())),
    }
}
...
fn get_ident_field_type(ident: &Ident, field_var: &Variable, scope: &ScopeVisitor) -> Result<Option<SoulType>, String> {
    ...
    match kind {
        ScopeKind::Class(multi_ref) => {
            let class_decl = multi_ref.borrow();
        ...
        ScopeKind::Struct(multi_ref) => {
            let struct_decl = multi_ref.borrow();
        ...
        ScopeKind::TypeDefed(multi_ref) => {
            field_type_from_type(&multi_ref.borrow().from_type.base, field_var, scope)
        },
        _ => unreachable!(),
    }
}
...
fn inner_get_fn_return_type(name: &str, callee: Option<SoulType>, generics: &Vec<SoulType>, arguments: &Vec<Arguments>, scope: &ScopeVisitor) -> Result<Option<SoulType>, String> {
    ...
    fn eq_calle(func: &FnDeclKind, calle: &Option<SoulType>) -> bool {
        func.get_signature().borrow().node.calle.as_ref().is_none_or(|this| calle.as_ref() == Some(&this.node.ty))
    }
    
    let func_with_same_calle = |kinds: &Vec<ScopeKind>, calle: &Option<SoulType>| kinds.iter().any(|kind| {
        
        match kind {
            ScopeKind::Functions(multi_ref) => multi_ref.borrow().iter().any(|func| eq_calle(func, calle)),
            _ => false,
        }
    });
    ...
    match scope.lookup_fn(name, func_with_same_calle, &callee) {
        ...
        Some(funcs) => funcs.iter().flat_map(|kind| match kind {
                ScopeKind::Functions(multi_ref) => multi_ref.borrow()
                    .iter()
                    .find(|func| eq_calle(func, &callee))
                    .map(|func| func.get_signature().borrow().node.return_type.clone()),
                _ => None,
