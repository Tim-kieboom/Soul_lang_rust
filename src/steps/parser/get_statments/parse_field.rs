...
pub fn try_get_field(stream: &mut TokenStream, scopes: &mut ScopeBuilder) -> Option<Result<Spanned<FieldDecl>>> {
    ...
    if END_TOKENS.iter().any(|sym| sym == stream.current_text()) {
        let field = FieldDecl{name: Ident(stream[name_i].text.clone()), ty, default_value: None, vis: FieldAccess::default() };
        let span = stream.current_span().combine(&stream[field_i].span);
        return Some(Ok(Spanned::new(field, span)));
    } 
    ...
    if stream.current_text() == "}" {
        ...
        let field = FieldDecl{name: Ident(stream[name_i].text.clone()), ty, default_value, vis: field_access };
        let span = stream.current_span().combine(&stream[field_i].span);
        return Some(Ok(Spanned::new(field, span)));
    }
